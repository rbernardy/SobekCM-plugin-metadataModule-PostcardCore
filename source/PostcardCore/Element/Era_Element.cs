...
{
    public class Era_Element : SobekCM.Library.Citation.Elements.simpleTextBox_Element
    {
        public PostcardCore_Logme pclogme = new PostcardCore_Logme();

        public Era_Element() : base("Era", "pc_era")
        {
            pclogme.logme("PC_Era_Element constructor...");
            Repeatable = false;
        }

        public override void Prepare_For_Save(SobekCM_Item Bib, User_Object Current_User)
        {
            pclogme.logme("PC_Era_Element: Prepare_For_Save...");
            // Do nothing since there is only one value.
        }

        public override void Render_Template_HTML(TextWriter Output, SobekCM_Item Bib, string Skin_Code, bool IsMozilla, StringBuilder PopupFormBuilder, User_Object Current_User, Web_Language_Enum CurrentLanguage, Language_Support_Info Translator, string Base_URL)
        {
            pclogme.logme("PC_Era_Element: Render_Tempalte_HTML...");

            if ( !String.IsNullOrEmpty(Acronym))
            {
                Acronym = "Enter the Era";
            }

            string valueToDisplay = string.Empty;

            // Look for an existing value for era
            PostcardCore_Info postcardInfo = Bib.Get_Metadata_Module("PostcardCore") as PostcardCore_Info;
            if (postcardInfo != null)
            {
                pclogme.logme("PC_Era_Element: Render_Template_HTML: rendering [" + postcardInfo.Era + "].");
                valueToDisplay = postcardInfo.Era;
            }
